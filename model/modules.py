    ...
    def __call__(self, in_tensor, nowbar=None, type_=None, reuse=False):
        ...
        if type_ is 0:  # 96 timesteps
            with tf.variable_scope(self.name, reuse=reuse):
                ...
                h3 = relu(batch_norm(transconv2d(h2, [16, 1], 256, kernels=[2, 1], strides=[2, 1], name='h3'), self.is_bn))

                #h4 = concat_prev(h3, nowbar[3] if nowbar else None)
                h4 = relu(batch_norm(transconv2d(h3, [32, 1], 128, kernels=[2, 1], strides=[2, 1], name='h4'), self.is_bn))

                #h5 = concat_prev(h4, nowbar[2] if nowbar else None)
                h5 = relu(batch_norm(transconv2d(h4, [96, 1], 128, kernels=[3, 1], strides=[3, 1], name='h5'), self.is_bn))

                #h6 = concat_prev(h5, nowbar[1] if nowbar else None)
                h6 = relu(batch_norm(transconv2d(h5, [96, 7], 64, kernels=[1, 7], strides=[1, 1], name='h6'), self.is_bn))

                #h7 = concat_prev(h6, nowbar[0] if nowbar else None)
                h7 = transconv2d(h6, [96, 84], self.output_dim, kernels=[1, 12], strides=[1, 12], name='h7')

            return tf.nn.tanh(h7)
            #return tf.nn.softmax(h7)
        
        elif type_ is 1:  # 48 timesteps concatenated all layers
            with tf.variable_scope(self.name, reuse=reuse):
...
class BarEncoder(object):
    ...
    def __call__(self, in_tensor, type_=None, reuse=False):
        ...
        if type_ is 0: # 96 timesteps
            with tf.variable_scope(self.name, reuse=reuse):
                h0 = lrelu(batch_norm(conv2d(in_tensor, 16, kernels=[1, 12], strides=[1, 12], name='h0'), self.is_bn)) #(?, 96, 7, 16)
                h1 = lrelu(batch_norm(conv2d(h0, 16, kernels=[1, 7], strides=[1, 7], name='h1'), self.is_bn)) #(?, 96, 1, 16)
                ...
                h6 = lrelu(batch_norm(conv2d(h5, 16, kernels=[2, 1], strides=[2, 1], name='h6'), self.is_bn)) #(?, 2, 1, 16)
    
            return [h0, h1, h2, h3, h4, h5, h6]
        
        elif type_ is 1: # 48 timesteps
            with tf.variable_scope(self.name, reuse=reuse):
    ...
    def __call__(self, in_tensor, nowbar=None, type_=None, reuse=False):
        ...
        if type_ is 0: # 96 timesteps
            with tf.variable_scope(self.name, reuse=reuse):
                ...
                ###################### paul115236 ######################
                ## conv
                h0 = lrelu(conv2d(in_tensor, 128, kernels=[1, 12], strides=[1, 12], name='h0')) # (64, 96, 7, 128)
                h1 = lrelu(conv2d(h0, 128, kernels=[1, 7], strides=[1, 7], name='h1')) # (64, 96, 1, 128)
                h2 = lrelu(conv2d(h1, 128, kernels=[2, 1], strides=[2, 1], name='h2')) # (64, 48, 1, 128)
                h3 = lrelu(conv2d(h2, 128, kernels=[2, 1], strides=[2, 1], name='h3')) # (64, 24, 1, 128)
                h4 = lrelu(conv2d(h3, 256, kernels=[4, 1], strides=[2, 1], name='h4')) # (64, 11, 1, 256)
                h5 = lrelu(conv2d(h4, 512, kernels=[3, 1], strides=[2, 1], name='h5')) # (64, 5, 1, 512)
