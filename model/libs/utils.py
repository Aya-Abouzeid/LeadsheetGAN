...
def to_image_np(bars):
    ...
    ##recolored_bars = np.matmul(bars.reshape(-1, 6), colormap).reshape((bars.shape[0], bars.shape[1], bars.shape[2], 3))
...
#     ##write_midi.write_piano_rolls_to_midi(images_with_pause_list, program_nums=[0,1,2,3,4,5], \
#     ##                                     is_drum=[False, True, False, False, False, False], filename=file_path, tempo=80.0)
## for 48 ts
def save_midis(bars, file_path):
    padded_bars = np.concatenate((np.zeros((bars.shape[0], bars.shape[1], 24, bars.shape[3])), bars, np.zeros((bars.shape[0], bars.shape[1], 20, bars.shape[3]))), axis=2) # here we supplement pitch 84(24-->107) --> 128(0-->127)
    pause = np.zeros((bars.shape[0], 48, 128, bars.shape[3]))
    images_with_pause = padded_bars
    images_with_pause = images_with_pause.reshape(-1, 48, padded_bars.shape[2], padded_bars.shape[3])
    images_with_pause_list = []
    for ch_idx in range(padded_bars.shape[3]):
        images_with_pause_list.append(images_with_pause[:,:,:,ch_idx].reshape(images_with_pause.shape[0],  \
                                                        images_with_pause.shape[1], images_with_pause.shape[2]))
